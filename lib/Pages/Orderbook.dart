...
class _OrderBookState extends State<OrderBook> {
  ...
  Future<void> saveOrder({
    ...
    final String url = 'http://10.0.2.2:3000/api/order/create';
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // Optionally fetch the orders again to update the list
        fetchOrders();
      } else {
  ...
  void fetchOrders() async {
    final String url = 'http://10.0.2.2:3000/api/order/wallet/${widget.walletId}';
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Sort the fetched orders into buyOrders and sellOrders
        for (var order in data) {
          if (order['action'] == 'buy') {
            buyOrders.add({
              'id': order['_id'],
              'price': order['price'],
              'volume': order['amount'], // Assuming 'amount' is the volume
            });
